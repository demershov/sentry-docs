...
Turns debug mode on or off. If debug is enabled, the SDK will attempt to print out debugging information. This can be useful for example if something goes wrong with sending the event. The default is always `False`. It's generally not recommended to turn it on in production because of the increase in log output.
